...
def compress_lzma(data):
    p = subprocess.Popen(['lzma', '-z', '-d23'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=False)
    r = p.communicate(data)
    return r[0]
...
class RomIoHeader(struct.Struct):
    ...
    def unpack(self, source):
        ...
        if self.signature != RomIoHeader.SIGNATURE:
            print("signature found %s", self.signature)
